...
function Layout({ children }) {
  ...
  useEffect(() => {
    if (context.x_token && context.current_room?.current != undefined) {
    ...
    if (!context.user_logged) {
      if (
        ...
        useNavigateWithCallback(
          navigate,
          "/login?callback_url=" + window.location.href,
        );
      } else {
  ...
  async function handleJoinRoomLink() {
    if (window.location.href.includes("roomAuth")) {
      ...
      await axios
        .post(
          `${config.baseUrl}rooms/verifyRoomToken/${context.user_logged?.id}`,
          { token: roomAuth },
          {
            headers: {
              Accept: "application/json",
